...
export function ServicesCTASection() {
  ...
  return (
    <section ref={sectionRef} aria-label="Services call to action" className={`${SERVICES_BG_SURFACE} ${SERVICES_SECTION_PAD}`}>
      <div className={SERVICES_INNER}>
        <div className="relative overflow-hidden rounded-[32px] border border-[#e8edf2] shadow-[0_20px_56px_rgba(15,23,42,0.14)] dark:border-border">
          <div className="absolute inset-0">
            <Image src={servicesCta.backgroundImage} alt="" fill className="object-cover" sizes="100vw" />
          </div>
          ...
          <div ref={contentRef} className="relative z-10 px-6 py-16 text-center sm:px-12 sm:py-20 lg:px-16 lg:py-24">
            ...
            <h2 data-cta-reveal className="mx-auto max-w-[760px] text-[32px] font-bold leading-[1.12] tracking-tight text-white sm:text-[40px] lg:text-[48px]">
            ...
            <div data-cta-reveal className="mt-8 flex flex-col flex-wrap items-center justify-center gap-3 sm:flex-row">
              {servicesCta.actions.map((action) => (
                <Link
                  key={action.label}
                  href={action.href}
                  className={`${action.primary ? "btn-primary" : "btn-secondary border-white/30 bg-white/10 text-white hover:bg-white/15"} inline-flex items-center justify-center gap-2 ${SERVICES_BTN_MOBILE} ${SERVICES_FOCUS_RING_LIGHT}`}
                >
                  {action.label}
