...
export default function ProductDetailHero({ product }: { product: ProductDetail }) {
  ...
  return (
    ...
    >
      <div ref={imageRef} className="absolute inset-0">
        <Image src={product.image} alt="" fill priority className={CINEMATIC_IMAGE} sizes="100vw" />
      </div>
      <div className="absolute inset-0 bg-gradient-to-t from-[#0a1628] via-[#0a1628]/70 to-[#0a1628]/35" />
      ...
      <div className={`${PRODUCTS_HERO_CONTENT} ${PRODUCTS_INNER}`}>
        <div ref={contentRef} className="max-w-3xl text-left">
          <nav aria-label="Breadcrumb" data-hero-reveal className={PRODUCTS_HERO_BREADCRUMB}>
            <ol className={`${PRODUCTS_HERO_BREADCRUMB_LIST} flex-wrap`}>
              <li>
                <Link href="/" className={`hover:text-white ${PRODUCTS_FOCUS_RING_LIGHT} rounded`}>Home</Link>
              </li>
              <li aria-hidden className="text-white/40"><ChevronRight className="h-3.5 w-3.5" /></li>
              <li>
                <Link href="/products" className={`hover:text-white ${PRODUCTS_FOCUS_RING_LIGHT} rounded`}>Products</Link>
              </li>
              <li aria-hidden className="text-white/40"><ChevronRight className="h-3.5 w-3.5" /></li>
              <li className="text-white/80">{product.category}</li>
              <li aria-hidden className="text-white/40"><ChevronRight className="h-3.5 w-3.5" /></li>
              <li className="max-w-[160px] truncate text-white/85 sm:max-w-xs" aria-current="page">{product.title}</li>
            </ol>
          </nav>
          ...
          <h1 data-hero-reveal className={`mb-4 ${PRODUCTS_HEADING_HERO}`}>{product.title}</h1>
